...
/**
 * V4: Search for real jobs using SERVERLESS WEB SCRAPING
 *
 * This version uses Puppeteer with @sparticuz/chromium for serverless:
 * - Scrapes real job boards: Indeed, LinkedIn, RemoteOK, ZipRecruiter, etc.
 * - Works in Netlify serverless functions (Chrome dynamically loaded)
 * - More comprehensive results than API-only approach
 * - Falls back to API search if scraping fails
 *
 * Flow:
 * 1. Web Scraping (15-25s) - Multiple job boards via Puppeteer
 * 2. Deduplication - Remove duplicate jobs across sources
 * 3. API Fallback - If scraping fails, use V3 API approach
 * 4. Mock Data - Only if everything fails
 *
...
async function searchJobsViaAI(queries: string[]): Promise<{ jobs: any[], usedMockData: boolean }> {
  console.log('🕷️  V4: Searching for real jobs via WEB SCRAPING...')
  ...
  try {
    ...
    // Search using Puppeteer scrapers (with serverless Chrome)
    // Limit to fast sources to avoid 30s timeout
    const result = await scraperJobSearchService.searchJobs(
      { keywords: primaryQuery },
      {
        sources: ['indeed', 'remoteok', 'ziprecruiter'], // Fast scrapers only
        parallel: true, // Run scrapers in parallel for speed
        maxResultsPerSource: 10, // Limit results to stay under 30s timeout
      }
    )
    ...
    if (result.jobs && result.jobs.length > 0) {
      ...
      console.log(`  ✓ Scraped ${transformed.length} jobs from ${result.sourcesUsed.length} sources: ${result.sourcesUsed.join(', ')}`)
    }

    if (allJobs.length === 0) {
      console.log('⚠️  Web scraping returned no jobs, falling back to API search...')

      // Fallback to V3 API approach
      const { jobSearchService } = await import('@/lib/services/job-search')
      const apiResult = await jobSearchService.searchJobs({ keywords: primaryQuery })

      if (apiResult.jobs && apiResult.jobs.length > 0) {
        const apiTransformed = apiResult.jobs.map(job => ({
          job_id: job.id,
          job_title: job.title,
          employer_name: job.company,
          job_city: job.location.split(',')[0]?.trim() || job.location,
          ...
          job_is_remote: job.type?.toLowerCase().includes('remote') || false,
          job_apply_link: job.url || '',
          job_publisher: job.source || 'API',
          job_posted_at_datetime_utc: job.posted_date || new Date().toISOString()
        }))

        allJobs.push(...apiTransformed)
        console.log(`  ✓ Fallback API found ${apiTransformed.length} jobs`)
      } else {
        console.log('⚠️  API fallback also failed, using mock data')
        return { jobs: getMockJobs(queries), usedMockData: true }
      }
    }

    console.log(`✅ V4 collected ${allJobs.length} total real jobs`)
    return { jobs: allJobs, usedMockData: false }

  } catch (error) {
    console.error('❌ V4 scraping failed:', error)
